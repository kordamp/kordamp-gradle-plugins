...
dependency(String):: Defines a dependency by name. This method takes an `Action`/`Closure` to define the dependency's
properties. Returns the named dependency (if it exists) if no additional arguments are given, throws an exception otherwise.
...
These dependencies can be used in combination with the <<org_kordamp_gradle_java_project_extension_cfg,cfg>> extension if
...
dependencies {
    ...
    // configure a platform module by groupId:argtifactId
