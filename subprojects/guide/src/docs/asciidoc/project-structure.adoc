...
`settings.gradle(.kts)` file to obtain the same behavior. With this structure in place the next step would be to setup
the minimum configuration on the root project's build file
