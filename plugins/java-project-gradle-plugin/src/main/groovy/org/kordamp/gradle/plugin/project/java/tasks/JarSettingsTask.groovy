 ...
 */
@CompileStatic
class JarSettingsTask extends AbstractSettingsTask {
    @TaskAction
    void report() {
        if (tasks) {
            tasks.each { t ->
                printTask((Jar) project.tasks.findByName(t))
            }
        } else if (task) {
            try {
                printTask((Jar) project.tasks.findByName(task))
            } catch (NullPointerException e) {
                throw new IllegalStateException("No matching '${this.task}' task was found")
            }
        } else {
            Set<Jar> compileTasks = new LinkedHashSet<>(project.tasks.withType(Jar))
            compileTasks.each { t ->
                printTask(t)
            }
        }
    ...
    private void printTask(Jar task) {
        print(task.name + ':', 0)
