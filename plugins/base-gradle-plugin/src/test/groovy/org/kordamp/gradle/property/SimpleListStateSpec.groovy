...
class SimpleListStateSpec extends Specification {
    @Rule
    ...
    def setup() {
        ...
        buildFile << """
            ...
            class StateTestTask extends DefaultTask {
                private final ListState input
            
                StateTestTask() {
                    input = SimpleListState.of(this, 'test.input', [])
                }
            
                @Internal
                ListProperty<String> getInput() {
                    input.property
                }
            
                @Input
                @Optional
                Provider<List<String>> getResolvedInput() {
                    input.provider
                }
    ...
    def "ENV_SYS_PROP with property set resolves property"() {
        given:
        buildFile << """
            project.tasks.stateTest.input = ['y1', 'y2']
            assert project.tasks.stateTest.resolvedInput.get() == ['y1', 'y2']
        """
    ...
    def "ENV_SYS_PROP with ENV_SYS_PROP set resolves env"() {
        given:
        buildFile << """
            assert project.tasks.stateTest.resolvedInput.get() == ['e1', 'e2']
        """
    ...
    def "ENV_SYS_PROP with SYS_PROP set resolves sys"() {
        given:
        buildFile << """
            assert project.tasks.stateTest.resolvedInput.get() == ['s1', 's2']
        """
    ...
    def "ENV_SYS_PROP with PROP set resolves prop"() {
        given:
        buildFile << """
            assert project.tasks.stateTest.resolvedInput.get() == ['p1', 'p2']
        """
