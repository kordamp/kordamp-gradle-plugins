...
public class MethodDescriptor implements Comparable<MethodDescriptor> {
    ...
    public boolean matches(@Nonnull MethodDescriptor md) {
        ...
        for (int i = 0; i < paramTypes.length; i++) {
            ...
            if (param1 != null && param2 != null && !ObjectUtils.isAssignableOrConvertibleFrom(param1, param2)) {
                return false;
            }
        }
    ...
    private static boolean areParametersCompatible(String[] params1, String[] params2) {
        ...
        for (int i = 0; i < params1.length; i++) {
            String p1 = params1[i];
            String p2 = params2[i];
            if (!p1.equals(p2) && !ObjectUtils.isMatchBetweenPrimitiveAndWrapperTypes(p1, p2)) {
                return false;
            }
        }
