...
class MutableSourceHtml extends AbstractFeature implements SourceHtml {
    final MutableConversion conversion
    final MutableOverview overview

    FileCollection srcDirs
    private final Set<Project> projects = new LinkedHashSet<>()

    MutableSourceHtml(Project project) {
        super(project)
        this.conversion = new MutableConversion(project)
        this.overview = new MutableOverview(project)
    ...
    void copyInto(MutableSourceHtml copy) {
        super.copyInto(copy)
    ...
    static void merge(MutableSourceHtml o1, MutableSourceHtml o2) {
        AbstractFeature.merge(o1, o2)
        MutableConversion.merge(o1.conversion, o2.conversion)
        MutableOverview.merge(o1.overview, o2.overview)
    ...
    void conversion(Action<? super MutableConversion> action) {
        action.execute(conversion)
    }

    void conversion(@DelegatesTo(MutableConversion) Closure action) {
        ConfigureUtil.configure(action, conversion)
    }

    void overview(Action<? super MutableOverview> action) {
        action.execute(overview)
    }

    void overview(@DelegatesTo(MutableOverview) Closure action) {
    ...
    static class MutableConversion implements Conversion {
        FileCollection srcDirs
        File destDir
        ...
        MutableConversion(Project project) {
        ...
        void copyInto(MutableConversion copy) {
        ...
        @CompileDynamic
        static void merge(MutableConversion o1, MutableConversion o2) {
    ...
    @EqualsAndHashCode(excludes = ['project'])
    @ToString(includeNames = true, excludes = ['project'])
    static class MutableOverview implements Overview {
        File destDir
        String pattern = '**/*.html'
        String windowTitle
        String docTitle
        ...
        MutableOverview(Project project) {
            this.project = project
        ...
        void copyInto(MutableOverview copy) {
            copy.destDir = destDir
        ...
        static void merge(MutableOverview o1, MutableOverview o2) {
            o1.destDir = o1.destDir ?: o2.destDir
